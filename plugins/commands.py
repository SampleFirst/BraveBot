...
from info import CHANNELS, ADMINS, AUTH_CHANNEL, UPDATE_CHANNEL, LOG_CHANNEL, PICS, BATCH_FILE_CAPTION, CUSTOM_FILE_CAPTION, PROTECT_CONTENT, CHNL_LNK, GRP_LNK, REQST_CHANNEL, SUPPORT_CHAT_ID, MAX_B_TN, IS_VERIFY, HOW_TO_VERIFY
from utils import add_new_user, get_settings, get_size, is_subscribed, is_updated, save_group_settings, temp, verify_user, check_token, check_verification, get_token, get_verify_status, send_all
...
async def start(client, message):
    ...
    if AUTH_CHANNEL and not await is_subscribed(client, message):
        try:
            invite_link = await client.create_chat_invite_link(int(AUTH_CHANNEL[0]))
        except ChatAdminRequired:
            logger.error("Mᴀᴋᴇ sᴜʀᴇ Bᴏᴛ ɪs ᴀᴅᴍɪɴ ɪɴ Fᴏʀᴄᴇsᴜʙ ᴄʜᴀɴɴᴇʟ")
            return
        btn = [
            [InlineKeyboardButton("❆ Jᴏɪɴ Oᴜʀ Bᴀᴄᴋ-Uᴘ Cʜᴀɴɴᴇʟ ❆", url=invite_link.invite_link)],
            [InlineKeyboardButton("❆ Jᴏɪɴ Oᴜʀ Bᴀᴄᴋ-Uᴘ Cʜᴀɴɴᴇʟ ❆", url="https://t.me/+bcWQ1MvJlqQ5YTE1")]
        ]
        ...
        return
    if UPDATE_CHANNEL and not await is_updated(client, message):
        try:
            ...
            return
        btn = [[
            InlineKeyboardButton("🎉 Join Update", url=invite_link.invite_link)
        ]]
        if message.command[1] != "subscribe":
            try:
                kk, file_id = message.command[1].split("_", 1)
        ...
        await client.send_message(
            chat_id=message.from_user.id,
            text="**Join Update Chat For Get Updates First**",
            reply_markup=InlineKeyboardMarkup(btn),
            parse_mode=enums.ParseMode.MARKDOWN
        )
        return
